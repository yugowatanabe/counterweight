document.addEventListener("DOMContentLoaded", function(event) {
  chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    ...
    while (title_unique !== title_unique.replace(re, " ")) {
      // do until no more words left to replace
    ...
    fetch(req).then(function(response) {
      response.json().then(function(obj) {
        ...
        var url_fetch_summary =
          "http://127.0.0.1:5000/article-summary?" + "url=" + source_url;
        ...
        fetch(req_summary).then(function(response) {
          response.json().then(function(obj) {
            ...
            if (returned_indices != undefined && returned_indices != null) {
              bg.console.log(
                "Now figure out what you want to do with the data"
              );
            ...
            // }
            chrome.storage.local.set({result: result}, function() {
...
function displayWithoutBias() {
  bg.console.log("displayWithout");
  var resultsList = document.getElementById('container');
  ...
  chrome.storage.local.get(["result"], function(get_result) {
    ...
    for (i = 0; i < 5; i++) {
      ...
      var time = result[i].publishedAt.match(
        /(.*)-(.*)-(.*)T(.*):(.*):(.*)Z/
      );
      var event = new Date(
        Date.UTC(
...
function displayWithBias(){
  bg.console.log("displayWith");
  chrome.storage.local.get(["source_biases","result"], function(get_result) {
    ...
    for (i = 0; i < result.length; i++) {
      ...
      if (biases_dict[source] !== undefined) {
        ...
        if (source_div == null) {
          ...
          let color = get_color(
            Math.round(biases_dict[source].quality) / 64
          );
          let r = color[0];
          let g = color[1];
          let b = color[2];
          source_q.innerHTML +=
            parse("<span style='color:rgba(%s,%s,%s);'>", r, g, b) +
            Math.round(biases_dict[source].quality) +
            "</span>";
    ...
    if (document.createEvent) {
      ...
      ticks[ticks.length - 1].fireEvent(
        "on" + event.eventType,
        event
      );
    }
